...
<br>
	<div class="panel panel-primary">
					<table class="table table-striped table-advance  table-bordered border-primary txt1" id="datatable"  >
                ...
                <tbody>
                  <c:forEach items="${itemBelowlist}" var="item">
						<tr>
						    <td>${item.id}</td>
							<td>${item.itemName}</td>
							<td>${item.description}</td>
							<td style="color:red;">${item.currentAmount}</td>
							<td>${item.measurement}</td>
							<!-- <td>${item.category}</td> -->
							
							<td style="color:red;">${item.threshold}</td>
						</tr>
					</c:forEach>
        </tbody>
